...
const PostForm: React.FC = () => {
    ...
    const makePost = async (e: React.MouseEvent) => {
        try {
            e.preventDefault()
            if(!isAuth) {
                ...
                <div className='flex-auto'>
                    <form className='flex flex-col gap-2 items-center justify-center mt-3'>
                        ...
                        <div className='flex justify-end w-full flex-row mt-3'>
                            <button onClick={makePost} className='bg-sky-500 rounded-full py-2 px-4 text-white text-xl font-semibold hover:bg-sky-700 cursor-pointer transition'>Tweet</button>
                        </div>
                    </form>
                </div>
            </div>
